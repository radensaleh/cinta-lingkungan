...
<head>
  ...
  <link rel="icon" type="image/png "href="{{ URL::asset('images/031-recycling-bin.png') }}">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="modal-form-add" action="{{ route('type-trash.store', 'tes') }}" method="post" role="form">
          {{ csrf_field() }}
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form id="modal-form-delete" method="post" action="{{route('type-trash.destroy','test')}}">
            {{ method_field('delete') }}
            {{ csrf_field() }}
      <div class="modal-body">
...
<div class="row">
  ...
  <tbody>
    @foreach($trash as $key => $data)
        <tr>
            ...
            <td>{{ $data->type }}</td>
            <td>
               <!-- <button class="btn btn-xs btn-info editmodal" value="{{ $data->id }}">Edit</button> -->
               <button class="btn btn-xs btn-warning" data-toggle="modal" data-target="#editData" data-id_trash = "{{ $data->id_trash }}" data-type_trash="{{ $data->type }}">
                 <span class="fa fa-edit"></span> Edit
               </button>
               <!-- <a class="btn btn-xs btn-danger" href="#">Hapus</a> -->
               <button class="btn btn-xs btn-danger" data-toggle="modal" data-target="#deleteData" data-id_trash = "{{ $data->id_trash }}">
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <form id="modal-form-edit" method="post" action="{{route('type-trash.update','test')}}">
              {{ method_field('patch') }}
              {{ csrf_field() }}
        <div class="modal-body">
              <input type="hidden" name="id_trash" id="cat_id" value="">
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
          ...
          // 					).then(OK => {
          //             if(OK){
          //               window.location.href = "{{ route('typetrash') }}";
          //             }
          //           });
          // 				} else{
          //           $('#addData').modal('hide');

...
<head>
  ...
  <link rel="icon" type="image/png "href="{{ URL::asset('images/026-warning.png') }}">
...
<aside class="app-sidebar" id="sidebar">
  <div class="sidebar-header">
    <a class="sidebar-brand" href="{{ route('index') }}"><span class="highlight">Cinta</span> Lingkungan</a>
      ...
      <ul class="nav navbar-nav navbar-right">
        ...
        <li class="dropdown notification danger">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">
            ...
            <div class="count">{{ $countTrash }}</div>
          </a>
          <div class="dropdown-menu">
            <ul>
              ...
              <li>
                <a href="#">
                  <span class="badge badge-danger pull-right">{{ $countTrash }}</span>
                  <div class="message">
                    <div class="content">
        ...
        <li class="dropdown profile">
          <a href="/html/pages/profile.html" class="dropdown-toggle"  data-toggle="dropdown">
            <img class="profile-img" src="{{ URL::asset('images/unknow.jpg') }}">
          ...
          <div class="form-group">
            ...
            <select class="form-control">
              @foreach($dataUser as $data)
                <option value="{{ $data->no_ktp }}">{{ $data->no_ktp }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
            ...
            <select class="form-control">
              @foreach($dataTrash as $data)
                <option value="{{ $data->id_trash }}">{{ $data->id_trash }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form id="modal-form-delete" method="post" action="{{route('data-complaint.destroy','test')}}">
            {{ method_field('delete') }}
            {{ csrf_field() }}
      <div class="modal-body">
...
<div class="row">
  ...
  <tbody>
    @foreach($dataComplaint as $data)
        <tr>
            <td>{{ $data->id_complaint }}</td>
            <td>{{ $data->no_ktp }}</td>
            <td>{{ $data->id_trash }}</td>
            <!-- <td>{{ $data->desc_complaint }}</td>
            <td>{{ $data->photo_complaint }}</td>
            <td>{{ $data->location }}</td> -->
            <td>{{ $data->date_complaint }}</td>
            <td>
                <button class="btn btn-xs btn-info" data-toggle="modal" data-target="#detailData" data-id_complaint="{{ $data->id_complaint }}" data-no_ktp="{{ $data->no_ktp }}" data-id_trash="{{ $data->id_trash }}" data-desc_complaint="{{ $data->desc_complaint }}" data-photo_complaint="{{ $data->photo_complaint }}" data-location="{{ $data->location }}" data-date_complaint="{{ $data->date_complaint }}">
                  <span class="fa fa-info-circle"></span> Detail
                </button>
                <!-- <button class="btn btn-xs btn-warning" data-toggle="modal" data-target="#editData" >
                  <span class="fa fa-edit"></span> Edit
                </button> -->
                <button class="btn btn-xs btn-danger" data-toggle="modal" data-target="#deleteData" data-id_complaint="{{ $data->id_complaint }}" >
                  ...
                  if( res.error == 0 ){
                    ...
                    swal(
                      ...
                      res.message,
                        ...
                        if(OK){
                          window.location.href = "{{ route('datacomplaint') }}";
                        }
                      });
                  } else{
                      $('#deleteData').modal('hide');
